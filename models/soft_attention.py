...
class SoftAtt(nn.Module):
    ...
    def forward(self, Out_six_rater, fea, out):
        ...
        for i in range(len(Out_six_rater)):
            ...
            pred_cup_list.append(torch.sigmoid(Out_six_rater[i][:, 1, :, :]).unsqueeze(1))
        ...
        uncertianty_disc = torch.std(pred_disc, dim=1).unsqueeze(1)
...
class SoftAtt_QUBIQ(nn.Module):
    def __init__(self):
        super(SoftAtt_QUBIQ, self).__init__()
        ...
        self.pred = nn.Conv2d(64 * 2, 1, kernel_size=3, padding=1)
        self.Soft = Soft()
    ...
    def forward(self, Out_raters, fea, out):
        fea = self.conv_block(fea)
        pred_disc_list = []

        for i in range(len(Out_raters)):
            pred_disc_list.append(torch.sigmoid(Out_raters[i][:, 0, :, :]).unsqueeze(1))
        ...
        fea_enhanced = torch.cat([soft_disc_fea, soft_u_disc_fea], dim=1)
        out = self.pred(fea_enhanced)

        return out, [Att_disc, uncertianty_disc]
